...
class Sandbox:
    def __init__(self, app: Flask, container_wall_timelimit, wall_timelimit: int, timelimit: int, memory_limit: int,
                 path: str, folder: str,
    ...
    def prepare(self):
        # Copy payload
        cp = subprocess.run('mkdir ' + self.path + self.folder + ' && cp ' + self.path +
                            '/Payload/* ' + self.path + self.folder + '&& chmod 777 ' + self.path +
                            self.folder, shell=True)
        if cp.returncode != 0:
            self.app.logger.error('COMMAND FAILED: ' + 'mkdir ' + self.path + self.folder + ' && cp ' + self.path +
                                  '/Payload/* ' + self.path + self.folder + '&& chmod 777 ' + self.path +
                                  self.folder)
        ...
        try:
            ...
            self.app.logger.error('FAILED TO WRITE/OPEN FILE: ' + code_file_path)
            return self.internal_error()
        try:
            ...
            self.app.logger.error('FAILED TO WRITE/OPEN FILE: ' + input_file_path)
            return self.internal_error()
    ...
    def execute(self):
        st = 'docker run --cap-add=ALL --privileged --rm -d' + ' -m 512M -i -t -v  "' + (
            os.getenv('TEMP_DIR') + '/' if os.getenv('TEMP_DIR')
             ...
             self.vm_name + ' /usercode/script.sh ' + self.lang['compilerScript'] + ' ' \
             + str(self.memory_limit) + ' ' + str(self.timelimit) + ' ' + str(self.wall_timelimit) + ' ' + \
             self.lang['runnerScript']
        self.app.logger.info('Docker start command: ' + st)
        container_id = subprocess.run(st, shell=True, stdout=subprocess.PIPE).stdout.decode("utf-8").rstrip('\n\r')
        try:
            subprocess.run('docker wait ' + str(container_id), timeout=self.container_wall_timelimit,
                           stderr=subprocess.PIPE,
                           shell=True,
                           stdout=subprocess.PIPE)
            subprocess.run('docker kill ' + str(container_id), stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                           shell=True)
        ...
        try:
            ...
            self.app.logger.error('FAILED TO WRITE/OPEN FILE: meta. some how sandbox failed')
            return self.internal_error()
        parsed_meta = dict()
        for i in meta.split('\n'):
            if i != '':
